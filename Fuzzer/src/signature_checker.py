...
class sigChecker():
    ...
    def read_sig(self, sigfile, xreg_idxes, freg_idxes,
        ...
        for csr_name in csr_names:
            ...
            #if csr_name == 'scause' or csr_name == 'mcause':
            #    #print(csr_name, csr_vals[csr_name])
            #    #csr_vals[csr_name] = csr_vals[csr_name] & 16
    ...
    def check(self, symbols):
        ...
        for csr_name in csr_names:
            ...
            if not match and csr_name=='mstatus':
                 if rtl_val == (isa_val | 1<<17): #check if setting MPRV in ISA mstatus make them equal
                      match = True
                      csr_match = True
                      print("[WARNING] mstatus mismatch due to MPRV mismatch ignored, Spike is going along with latest spec")
            if not match and csr_name=='scause':
                 if rtl_val == (isa_val | 1<<4): #check if setting 5th bit in ISA scause make them equal
                      match = True
                      csr_match = True
                      print("[WARNING] scause mismatch due to BOOM allowing 5 bit value")
            if not match and csr_name=='stval':
                 if rtl_val == 0: #check if setting 5th bit in ISA scause make them equal
                      match = True
                      csr_match = True
                      print("[WARNING] stval mismatch possibly on illegal instr ignored")
            if not match and csr_name in ['mstatus','sstatus']:
                 #print('[ISA] {:016x} || [RTL] {:016x}'.format(isa_val| 1<<13|1<<63,rtl_val))
                 if rtl_val == (isa_val | 1<<13 | 1<<63) or rtl_val == (isa_val | 1<<14 | 1<<63): 
