...
def wait_check(proc_pid_arr: list, manager: procManager):
    ...
    if exit_code != 0:
        print('[ProcessorFuzz] Something bad happens! exit_code: {}'.format(exit_code))
    ...
    if state != proc_state.NORMAL:
        print('[ProcessorFuzz] Child {} is in abnormal state {}!'.format(idx, proc_state.tpe[state]))
        for pid in proc_pid_arr:
            if pid != end_pid:
                os.kill(pid, signal.SIGKILL)
        exit(-1)
...
parser.add_option('record', 1, 'Record the result')
...
if not multicore:
    ...
    print('[ProcessorFuzz] Muticore Fuzzing, Number of batches -- {}'.format(num_batch))
    ...
    for i in range(num_batch * multicore):
        ...
        print('[ProcessorFuzz] Multicore Fuzzing Batch [{}], Thread [{}]'.format(i, proc_num))
    ...
    if not parent:
        ...
        print('[ProcessorFuzz] Stop Multicore Fuzzing')
